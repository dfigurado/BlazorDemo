...
{
    public class HttpClientHelper(IHttpClientFactory httpClientFactory, LocalStorageService localStorageService)
    {
        ...
        public HttpClient GetPublicHttpClient(string token)
        {
            var client = httpClientFactory.CreateClient("SystemApiClient");
            client.DefaultRequestHeaders.Remove(HeaderKey);
            return client;
